  ...
  function handleSocial(provider) {
    alert(`${provider} OAuth is not yet connected to the backend.`);
  }
  ...
  function updateLoginBtn() {
    btnLogin.disabled = !(isValidEmail(loginEmail.value) && loginPassword.value.length >= 8);
  }

  loginEmail.addEventListener('input', updateLoginBtn);
  loginPassword.addEventListener('input', updateLoginBtn);
  loginEmail.addEventListener('blur', () => {
    if (loginEmail.value && !isValidEmail(loginEmail.value)) showError(loginEmailErr, 'Enter a valid email address');
  ...
  document.getElementById('form-login').addEventListener('submit', async (e) => {
    ...
    if (!emailOk) { showError(loginEmailErr, 'Enter a valid email address'); }
    ...
    try {
      ...
      if (!response.ok) {
        showError(loginPwErr, (typeof result === 'string' && result.trim()) ? result : 'Invalid email or password');
        return;
      }

      const user = typeof result === 'object' && result !== null ? result : {};
      localStorage.setItem('userEmail', user.email || loginEmail.value.trim());
  ...
  function updateSignupBtn() {
    ...
    btnSignup.disabled = !(nameOk && emailOk && pwOk && matchOk);
  }
